...
const validApiUrl = "https://demo.eu.holipoly.cloud/graphql/";
...
describe("verifyJWT", () => {
  ...
  it("Process valid request", async () => {
    await verifyJWT({ appId: validAppId, holipolyApiUrl: validApiUrl, token: validToken });
  });

  it("Throw error on decode issue", async () => {
    await expect(
      verifyJWT({ appId: validAppId, holipolyApiUrl: validApiUrl, token: "wrong_token" })
    ).rejects.toThrow("JWT verification failed: Could not decode authorization token.");
  });

  it("Throw error on app ID missmatch", async () => {
    await expect(
      verifyJWT({ appId: "wrong_id", holipolyApiUrl: validApiUrl, token: validToken })
