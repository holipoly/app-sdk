  ...
  switch (event.type) {
    case EventType.handshake: {
      ...
      return {
        ...
        holipolyVersion: event.payload.holipolyVersion,
...
const getHolipolyApiUrlFromUrl = () =>
  new URL(window.location.href).searchParams.get(AppIframeParams.HOLIPOLY_API_URL) || "";
...
const getDefaultOptions = (): AppBridgeOptions => ({
  ...
  holipolyApiUrl: getHolipolyApiUrlFromUrl(),
...
export class AppBridge {
  ...
  constructor(options: AppBridgeOptions = {}) {
    ...
    if (!this.combinedOptions.holipolyApiUrl) {
      debug("?holipolyApiUrl was not found in iframe url");
    }
    ...
    if (!(this.combinedOptions.holipolyApiUrl || this.combinedOptions.targetDomain)) {
      console.error(
        "domain and holipolyApiUrl params were not found in iframe url. Ensure at least one of them is present"
      );
    }
  ...
  private setInitialState() {
    ...
    const state: Partial<AppBridgeState> = {
      ...
      holipolyApiUrl: this.combinedOptions.holipolyApiUrl,
