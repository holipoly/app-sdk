...
describe("create-app-register-handler", () => {
  ...
  it("Sets auth data for correct request", async () => {
    ...
    expect(mockApl.set).toHaveBeenCalledWith({
      holipolyApiUrl: "https://mock-holipoly-domain.holipoly.cloud/graphql/",
      domain: "https://mock-holipoly-domain.holipoly.cloud/",
  ...
  it("Returns 403 if configured to work only for specific holipoly URL and try to install on prohibited one", async () => {
    const { res, req } = createMocks({
      /**
       * Use body, instead of params, otherwise - for some reason - param is not accessible in mock request
       * Maybe this is a bug https://github.com/howardabrams/node-mocks-http/blob/master/lib/mockRequest.js
      ...
      headers: {
        host: "some-holipoly-host.cloud",
        "x-forwarded-proto": "https",
        "holipoly-api-url": "https://wrong-holipoly-domain.holipoly.cloud/graphql/",
        "holipoly-domain": "https://wrong-holipoly-domain.holipoly.cloud/",
      },
      method: "POST",
    });
    ...
    const handler = createAppRegisterHandler({
      apl: mockApl,
      allowedHolipolyUrls: [(url: string) => url === "https://mock-holipoly-domain.holipoly.cloud"],
    });
    ...
    it("Runs callback hooks - successful saving to APL scenario", async () => {
      ...
      expect(mockOnRequestStart).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          holipolyDomain: "https://mock-holipoly-domain.holipoly.cloud/",
          holipolyApiUrl: "https://mock-holipoly-domain.holipoly.cloud/graphql/",
        })
      );
