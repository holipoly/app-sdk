...
import { HolipolyAsyncWebhook } from "./holipoly-async-webhook";
import { NextWebhookApiHandler, WebhookConfig } from "./holipoly-webhook";
import { processHolipolyWebhook } from "./process-holipoly-webhook";

const webhookPath = "api/webhooks/product-updated";
const baseUrl = "http://example.com";

describe("HolipolyAsyncWebhook", () => {
  ...
  const holipolyAsyncWebhook = new HolipolyAsyncWebhook(validAsyncWebhookConfiguration);

  it("constructor passes if query is provided", async () => {
    expect(() => {
      // eslint-disable-next-line no-new
      new HolipolyAsyncWebhook({
  ...
  it("targetUrl should return full path to the webhook route based on given baseUrl", async () => {
    expect(holipolyAsyncWebhook.getWebhookManifest(baseUrl)).toEqual(
  ...
  it("getWebhookManifest should return a valid manifest", async () => {
    expect(holipolyAsyncWebhook.getWebhookManifest(baseUrl)).toStrictEqual({
  ...
  it("Test createHandler which return success", async () => {
    ...
    const wrappedHandler = holipolyAsyncWebhook.createHandler(testHandler);
  ...
  it("Calls callbacks for error handling", async () => {
    ...
    const webhook = new HolipolyAsyncWebhook({
      ...validAsyncWebhookConfiguration,
