...
import {
  HolipolyWebhookError,
  processHolipolyWebhook,
  WebhookContext,
  WebhookError,
} from "./process-holipoly-webhook";

const debug = createDebug("HolipolyWebhook");
...
export const WebhookErrorCodeMap: Record<HolipolyWebhookError, number> = {
  OTHER: 500,
...
export abstract class HolipolyWebhook<
  TPayload = unknown,
...
> {
  ...
  /**
   * Wraps provided function, to ensure incoming request comes from registered Holipoly instance.
  ...
  createHandler(handlerFn: NextWebhookApiHandler<TPayload, TExtras>): NextApiHandler {
    return async (req, res) => {
      ...
      await processHolipolyWebhook<TPayload>({
        req,
        apl: this.apl,
