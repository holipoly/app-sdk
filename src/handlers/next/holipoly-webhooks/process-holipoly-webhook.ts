...
const debug = createDebug("processHolipolyWebhook");

export type HolipolyWebhookError =
  | "OTHER"
  | "MISSING_HOST_HEADER"
...
export class WebhookError extends Error {
  errorType: HolipolyWebhookError = "OTHER";

  constructor(message: string, errorType: HolipolyWebhookError) {
    super(message);
    if (errorType) {
...
export type WebhookContext<T> = {
  ...
  payload: T;
  authData: AuthData;
  /** For Holipoly < 3.15 it will be null. */
  schemaVersion: number | null;
};

interface ProcessHolipolyWebhookArgs {
  req: NextApiRequest;
  apl: APL;
  allowedEvent: string;
}

type ProcessHolipolyWebhook = <T = unknown>(
  props: ProcessHolipolyWebhookArgs
 ...
 */
export const processHolipolyWebhook: ProcessHolipolyWebhook = async <T>({
  req,
  apl,
  allowedEvent,
}: ProcessHolipolyWebhookArgs): Promise<WebhookContext<T>> => {
  ...
  return tracer.startActiveSpan(
    ...
    async (span) => {
      try {
        ...
        const { event, signature, holipolyApiUrl } = getHolipolyHeaders(req.headers);
        ...
        if (!holipolyApiUrl) {
          ...
          throw new WebhookError("Missing holipoly-api-url header", "MISSING_API_URL_HEADER");
        }

        if (!event) {
          debug("Missing holipoly-event header");
          throw new WebhookError("Missing holipoly-event header", "MISSING_EVENT_HEADER");
        }
        ...
        if (!signature) {
          ...
          throw new WebhookError("Missing holipoly-signature header", "MISSING_SIGNATURE_HEADER");
        }
        ...
        /**
         * Verify if the app is properly installed for given Holipoly API URL
         */
        const authData = await apl.get(holipolyApiUrl);

        if (!authData) {
          debug("APL didn't found auth data for %s", holipolyApiUrl);

          throw new WebhookError(
            `Can't find auth data for ${holipolyApiUrl}. Please register the application`,
            "NOT_REGISTERED"
          );
        }
        ...
        try {
          ...
          const newJwks = await fetchRemoteJwks(authData.holipolyApiUrl).catch((e) => {
            debug(e);
