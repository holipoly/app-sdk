...
export class EnvAPL implements APL {
  ...
  private isAuthDataValid(authData: AuthData): boolean {
    const keysToValidateAgainst: Array<keyof AuthData> = ["appId", "holipolyApiUrl", "token"];
  ...
  async get(holipolyApiUrl: string) {
    ...
    if (holipolyApiUrl !== this.options.env.holipolyApiUrl) {
      throw new Error(
        `Requested AuthData for domain "${holipolyApiUrl}", however APL is configured for ${this.options.env.holipolyApiUrl}. You may trying to install app in invalid Holipoly URL `
      );
    }
  ...
  async getAll() {
    ...
    const authData = await this.get(this.options.env.holipolyApiUrl);
