...
export class FileAPL implements APL {
  ...
  constructor(config: FileAPLConfig = {}) {
    this.fileName = config?.fileName || ".holipoly-app-auth.json";
  }
  ...
  private async loadDataFromFile(): Promise<AuthData | undefined> {
    ...
    const { token, domain, holipolyApiUrl, appId, jwks } = parsedData;

    if (token && holipolyApiUrl && appId) {
      debug("Token and domain found, returning values: %s, %s", domain, `${token[0]}***`);

      const authData: AuthData = { token, holipolyApiUrl, appId };
  ...
  async get(holipolyApiUrl: string) {
    ...
    if (holipolyApiUrl === authData?.holipolyApiUrl) {
      return authData;
    }
    return undefined;
  }
