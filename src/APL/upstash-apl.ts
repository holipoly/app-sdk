...
export class UpstashAPL implements APL {
  ...
  private async upstashRequest(request: string[]) {
    ...
    if (!this.restURL || !this.restToken) {
      throw new Error(
        "UpstashAPL is not configured. See https://docs.holipoly.io/docs/3.x/developer/extending/apps/developing-apps/app-sdk/apl"
      );
    }
  ...
  private async saveDataToUpstash(authData: AuthData) {
    ...
    await this.upstashRequest(["SET", authData.holipolyApiUrl, data]);
  }

  private async deleteDataFromUpstash(holipolyApiUrl: string) {
    await this.upstashRequest(["DEL", holipolyApiUrl]);
  }

  private async fetchDataFromUpstash(holipolyApiUrl: string) {
    const result = await this.upstashRequest(["GET", holipolyApiUrl]);
    if (result) {
  ...
  async get(holipolyApiUrl: string) {
    return this.fetchDataFromUpstash(holipolyApiUrl);
  }
  ...
  async delete(holipolyApiUrl: string) {
    await this.deleteDataFromUpstash(holipolyApiUrl);
  }
