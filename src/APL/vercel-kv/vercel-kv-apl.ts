...
export class VercelKvApl implements APL {
  ...
  async get(holipolyApiUrl: string): Promise<AuthData | undefined> {
    this.debug("Will call Vercel KV to get auth data for %s", holipolyApiUrl);
    ...
    return this.tracer.startActiveSpan(
      ...
      async (span) => {
        try {
          const authData = await kv.hget<AuthData>(this.hashCollectionKey, holipolyApiUrl);

          this.debug("Received response from VercelKV");

          if (!authData) {
            this.debug("AuthData is empty for %s", holipolyApiUrl);
          }
  ...
  async set(authData: AuthData): Promise<void> {
    this.debug("Will call Vercel KV to set auth data for %s", authData.holipolyApiUrl);
      ...
      async (span) => {
        try {
          await kv.hset(this.hashCollectionKey, {
            [authData.holipolyApiUrl]: authData,
          });
  ...
  async delete(holipolyApiUrl: string) {
    this.debug("Will call Vercel KV to delete auth data for %s", holipolyApiUrl);
    ...
    return this.tracer.startActiveSpan(
      ...
      async (span) => {
        try {
          await kv.hdel(this.hashCollectionKey, holipolyApiUrl);
