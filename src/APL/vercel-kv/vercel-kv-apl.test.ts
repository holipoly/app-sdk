...
const getMockAuthData = (holipolyApiUrl = "https://demo.holipoly.io/graphql"): AuthData => ({
  appId: "foobar",
  holipolyApiUrl,
  token: "token",
  domain: "domain",
...
describe("VercelKvApl", () => {
  ...
  describe("get", () => {
    it("returns parsed auth data", async () => {
      ...
      const authData = await apl.get("https://demo.holipoly.io/graphql");
  ...
  describe("set", () => {
    it("Sets auth data under a namespace provided in env", async () => {
      ...
      expect(kv.hset).toHaveBeenCalledWith(APP_NAME_NAMESPACE, {
        "https://demo.holipoly.io/graphql": getMockAuthData(),
      });
    });
  });
});
