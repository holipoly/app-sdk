  ...
  describe("FileAPL", () => {
    describe("get", () => {
      it("Should fallback to 'undefined' if parsing fail fails", async () => {
        ...
        await expect(apl.get(stubAuthData.holipolyApiUrl)).resolves.toBe(undefined);
      });
      ...
      it("Returns auth data if domain and JWKS are missing", async () => {
        vi.spyOn(fsPromises, "readFile").mockResolvedValue(
          JSON.stringify({
            ...
            holipolyApiUrl: stubAuthData.holipolyApiUrl,
        ...
        expect(await apl.get(stubAuthData.holipolyApiUrl)).toStrictEqual({
          token: stubAuthData.token,
          holipolyApiUrl: stubAuthData.holipolyApiUrl,
          appId: stubAuthData.appId,
        });
    ...
    describe("set", () => {
      it("Handle write file errors", async () => {
        ...
        expect(spyWriteFile).toBeCalledWith(
          ".holipoly-app-auth.json",
          JSON.stringify(stubAuthData)
        );
      });
    ...
    describe("delete", () => {
      it("Should override file when called with known domain", async () => {
        ...
        await apl.delete(stubAuthData.holipolyApiUrl);

        expect(spyWriteFile).toBeCalledWith(".holipoly-app-auth.json", "{}");
      });
