...
- 7e9b259: Added VercelKV APL. It provides out-of-the-box integration. See [docs](https://docs.holipoly.io/developer/extending/apps/developing-apps/app-sdk/apl#vercelkvapl)

## 0.50.3

### Patch Changes

- ac4e4a2: Now, the fixed body size limit was removed, this means there is no restriction for size of the webhook payload, sent by Holipoly.
...
- ef61335: Fix wrong logic introduced in [0.49.0](https://github.com/holipoly/app-sdk/releases/tag/v0.49.0): there is not header `holipoly-schema-version` when app-sdk is processing holipoly webhook. This header is only present on install request.

  Now app-sdk will try to parse version from `version` field on GraphQL subscription [Event](https://docs.holipoly.io/docs/3.x/api-storefront/miscellaneous/interfaces/event#code-style-fontweight-normal-eventbversionbcodestring-). If field is not present `null` will be returned.

## 0.49.0

### Minor Changes

- 53b8cdc: Parse the `holipoly-schema-version` header and include the parsed version in the contexts of `createHandler` and `webhookFactory`. If the header is absent (Holipoly version below 3.15), the version will default to `null`. This parsed version enables supporting multiple schemas in a single app, as outlined in the [RFC](https://github.com/holipoly/apps/issues/1213).
...
- f49c63f: Added VercelKvApl. It will use [Vercel KV Storage](https://vercel.com/docs/storage/vercel-kv) (using Upstash Redis under the hood).
  ...
  ```
  import { _experimental_VercelKvApl } from "@holipoly/app-sdk/apl";
  ```
...
- 3c68c4c: Added definition for payment and transaction events. From now on, sync webhook factory will autocomplete response expected by Holipoly
...
- 3e31b3a: Changed the "query" field in the AppManifest webhook to be required. Previously, this field was optional.
  ...
  For subscription events, Holipoly rejects webhooks without query, so this field was valid only with legacy non-subscription webhooks.
...
- 8ff42af: Added export path "@holipoly/app-sdk/types". It was accessible but not correctly marked in package.json. Now its added in "exports" field which should fix some bundles and IDEs
...
- 1118ea9: Added new actions and events for dynamic permissions request.
  ...
  This feature is available in Holipoly 3.15 and higher
...
- b8935a8: Exposed "@holipoly/app-sdk/headers" path. It contains helper methods: getHolipolyHeaders and getBaseUrl
...
- b365c7c: Added holipolyVersion and dashboardVersion fields to AppBridge state. They are optional - will be provided from 3.15.
...
- 1d08329: Added "brand" field to Manifest type, that allows to provide app's logo URL. It will work from Holipoly 3.15
...
- d4169dc: Added new method to SettingsManager - "delete". It can delete metadata by key in Holipoly. Implemented in MetadataManager and EncryptedMetadataManager
...
- f809368: Added `tokenRefresh` event to AppBridge.
  ...
  This fixes [this issue](https://github.com/holipoly/holipoly-app-sdk/issues/222)

  For Holipoly Cloud where token lives for 24h it was rare, but Holipoly can be set to have any token duration, causing app to fail fast.
...
- 62e4c39: Registed Handler now prints holipolyApiUrl in error response, so its available in dashboard toast
- 62e4c39: Wrapped useAuthenticatedFetch with useMemo to avoid re-renders
- c777275: Fixed HolipolyCloudAPL "getAll" method that was not mapping response from remote with AuthData interface
- c777275: Added additional debug log if holipolyApiUrl doesnt exist in register handler
...
- 5744aa4: Add requiredHolipolyVersion field to AppManifest, so Holipoly can validate it during installation
...
- dfd632b: "domain" field in AuthData is no longer required. It will be set by registerHandler, but if missing, domain can be resolved from holipolyApiUrl
...
- 9420209: Add HolipolySyncWebhook class

  Now app-sdk support both `new HolipolyAsyncWebhook()` and `new HolipolySyncWebhook()`.

  Changes:

  Constructor field `subscriptionQueryAst?: ASTNode` in `HolipolyAsyncWebhook` has been deprecated.
